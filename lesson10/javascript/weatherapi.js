...
fetch(`http://api.openweathermap.org/data/2.5/weather?id=${PRESTION_ID}&appid=${APPID}&units=imperial`)
  .then((response) => response.json())
  .then((json_data) => {
    document.querySelector('span#temperature').textContent = json_data["main"]["temp"] + '°F';

    let image = document.querySelector('img#icon');
    image_source = `http://api.openweathermap.org/img/w/${json_data['weather'][0]['icon']}.png`;
